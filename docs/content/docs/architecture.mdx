...
Each subsystem defines its own store interface independently. A single backend implementation satisfies all of them. The memory, PostgreSQL, Grove, SQLite, and Redis backends all implement this interface.
