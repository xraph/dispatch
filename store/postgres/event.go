...
// Uses a polling approach with short intervals since grove is dialect-agnostic
...
func (s *Store) SubscribeEvent(ctx context.Context, name string, timeout time.Duration) (*event.Event, error) {
	...
	for {
		...
		if convErr != nil {
			return nil, fmt.Errorf("dispatch/postgres: subscribe event convert: %w", convErr)
		}
		return evt, nil
	}
}
...
func (s *Store) AckEvent(ctx context.Context, eventID id.EventID) error {
	res, err := s.pgdb.NewUpdate((*eventModel)(nil)).
		Set("acked = TRUE").
