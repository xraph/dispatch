...
func (s *Store) RegisterWorker(ctx context.Context, w *cluster.Worker) error {
	...
	_, err := s.pgdb.NewInsert(m).
		...
		Exec(ctx)
	if err != nil {
		return fmt.Errorf(errPrefix+"register worker: %w", err)
	}
	return nil
}
...
func (s *Store) DeregisterWorker(ctx context.Context, workerID id.WorkerID) error {
	res, err := s.pgdb.NewDelete((*workerModel)(nil)).
		...
		Exec(ctx)
	if err != nil {
		return fmt.Errorf(errPrefix+"deregister worker: %w", err)
	}
...
func (s *Store) HeartbeatWorker(ctx context.Context, workerID id.WorkerID) error {
	res, err := s.pgdb.NewUpdate((*workerModel)(nil)).
		...
		Exec(ctx)
	if err != nil {
		return fmt.Errorf(errPrefix+"heartbeat worker: %w", err)
	}
	if rows, _ := res.RowsAffected(); rows == 0 { //nolint:errcheck // driver always returns nil
		return dispatch.ErrWorkerNotFound
	}
...
func (s *Store) ListWorkers(ctx context.Context) ([]*cluster.Worker, error) {
	...
	err := s.pgdb.NewSelect(&models).
		...
		Scan(ctx)
	if err != nil {
		return nil, fmt.Errorf(errPrefix+"list workers: %w", err)
	}
	...
	for i := range models {
		...
		if convErr != nil {
			return nil, fmt.Errorf(errPrefix+"list workers convert: %w", convErr)
		}
...
func (s *Store) DeleteStaleWorkers(ctx context.Context, threshold time.Duration) (int64, error) {
	res, err := s.pgdb.NewDelete((*workerModel)(nil)).
		...
		Exec(ctx)
	if err != nil {
		return 0, fmt.Errorf(errPrefix+"delete stale workers: %w", err)
	}
	n, err := res.RowsAffected()
	if err != nil {
		return 0, fmt.Errorf(errPrefix+"delete stale workers rows affected: %w", err)
	}
	return n, nil
}
...
func (s *Store) ReapDeadWorkers(ctx context.Context, threshold time.Duration) ([]*cluster.Worker, error) {
	...
	err := s.pgdb.NewSelect(&models).
		...
		Scan(ctx)
	if err != nil {
		return nil, fmt.Errorf(errPrefix+"reap dead workers: %w", err)
	}
	...
	for i := range models {
		w, convErr := fromWorkerModel(&models[i])
		if convErr != nil {
			return nil, fmt.Errorf(errPrefix+"reap dead workers convert: %w", convErr)
		}
		workers = append(workers, w)
	}
	return workers, nil
...
func (s *Store) AcquireLeadership(ctx context.Context, workerID id.WorkerID, ttl time.Duration) (bool, error) {
	...
	_, err := s.pgdb.NewUpdate((*workerModel)(nil)).
		...
		Exec(ctx)
	if err != nil {
		return false, fmt.Errorf(errPrefix+"clear expired leader: %w", err)
	}
	...
	if err != nil {
		if !isNoRows(err) {
			return false, fmt.Errorf(errPrefix+"check leader: %w", err)
		}
	...
	res, claimErr := s.pgdb.NewUpdate((*workerModel)(nil)).
		...
		Exec(ctx)
	if claimErr != nil {
		return false, fmt.Errorf(errPrefix+"claim leadership: %w", claimErr)
	}
	if rows, _ := res.RowsAffected(); rows == 0 { //nolint:errcheck // driver always returns nil
		return false, nil
	}
...
func (s *Store) RenewLeadership(ctx context.Context, workerID id.WorkerID, ttl time.Duration) (bool, error) {
	...
	res, err := s.pgdb.NewUpdate((*workerModel)(nil)).
		...
		Exec(ctx)
	if err != nil {
		return false, fmt.Errorf(errPrefix+"renew leadership: %w", err)
	}
	if rows, _ := res.RowsAffected(); rows == 0 { //nolint:errcheck // driver always returns nil
		return false, nil
	}
...
func (s *Store) GetLeader(ctx context.Context) (*cluster.Worker, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf(errPrefix+"get leader: %w", err)
	}
