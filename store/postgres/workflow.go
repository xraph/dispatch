...
func (s *Store) ListRuns(ctx context.Context, opts workflow.ListOpts) ([]*workflow.Run, error) {
	...
	for i := range models {
		...
		if convErr != nil {
			return nil, fmt.Errorf("dispatch/postgres: list runs convert: %w", convErr)
		}
		runs = append(runs, r)
	}
	return runs, nil
...
func (s *Store) ListCheckpoints(ctx context.Context, runID id.RunID) ([]*workflow.Checkpoint, error) {
	...
	for i := range models {
		...
		if convErr != nil {
			return nil, fmt.Errorf("dispatch/postgres: list checkpoints convert: %w", convErr)
		}
...
func (s *Store) ListChildRuns(ctx context.Context, parentRunID id.RunID) ([]*workflow.Run, error) {
	...
	err := s.pgdb.NewSelect(&models).
		...
		Scan(ctx)
	if err != nil {
		return nil, fmt.Errorf("dispatch/postgres: list child runs: %w", err)
	}
	...
	for i := range models {
		r, convErr := fromRunModel(&models[i])
		if convErr != nil {
			return nil, fmt.Errorf("dispatch/postgres: list child runs convert: %w", convErr)
		}
		runs = append(runs, r)
	}
	return runs, nil
...
func (s *Store) DeleteCheckpointsAfter(ctx context.Context, runID id.RunID, afterStep string) error {
	_, err := s.pgdb.NewDelete(&checkpointModel{}).
		...
		Exec(ctx)
	if err != nil {
		return fmt.Errorf("dispatch/postgres: delete checkpoints after: %w", err)
	}
	return nil
}
