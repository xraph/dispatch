...
func (s *Store) ListChildRuns(ctx context.Context, parentRunID id.RunID) ([]*workflow.Run, error) {
	...
	err := s.sdb.NewSelect(&models).
		...
		Scan(ctx)
	if err != nil {
		return nil, fmt.Errorf("dispatch/sqlite: list child runs: %w", err)
	}
	...
	for i := range models {
		r, convErr := fromRunModel(&models[i])
		if convErr != nil {
			return nil, fmt.Errorf("dispatch/sqlite: list child runs convert: %w", convErr)
		}
		runs = append(runs, r)
	}
	return runs, nil
